...
def hm_runners(df):
    ...
    S = (df['draw'] == 0).astype(int).sum(axis=1)
    df.insert(loc=0, column='hm_runners', value=14 - S)
    return df

def to_3_value(pred,real,df,match_race_id_from_indices):
    
    'return 3 or 2 values for prediciton and real value for placed horses depending on the numbers of runners'
...
def get_place_odds(df,race_id,horse_no):
    ...
    return A # we could put a condition oif the value does not exist and return 1 by default but we prefer discard this bet
...
def compute_df_placed_xgb(pred,real,df,match_id):
    'return a df with information to draw the evolution of our investement'
    #pred = to_3_value(pred)
    #real = to_3_value(real.values)
    L = []
    for i in range(len(real)):
        L.append([get_place_odds(df,match_id[i],real[i][j]) for j in range(len(real[i]))])
...
def mean_place_odds(X_test_init):
    "function which return the emean place odd for all race and all horse for this month"
    R = X_test_init.place_odds.values
    R
    aa = []
